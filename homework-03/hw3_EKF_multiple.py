...
if __name__ == "__main__":
    ...
    trajectories_z1_followed_by_z2 = []
    trajectories_z1_stacking_with_z2 = []
    
    for init_mu in attempts:
      init_mu = np.expand_dims(np.array(init_mu), axis=1)
      ...
      trajectories_z1_followed_by_z2.append(np.vstack((trajectories_with_z1, trajectories_with_z2)))
      ...
      trajectories_z1_stacking_with_z2.append(trajectories)
    
      
    def converge_visualization(trajectories, save_name):
        ...
        for i in range(len(xs)):
            axs[0].plot(t, xs[i], label=f'Line {i+1}')
        axs[0].legend(loc="upper right")
        axs[0].set_title("x")

        axs[1] = plt.subplot(312) 
        for i in range(len(ys)):
            axs[1].plot(t, ys[i], label=f'Line {i+1}')
        axs[1].legend(loc="upper right")
        axs[1].set_title("y")
        
        axs[2] = plt.subplot(313) 
        for i in range(len(zs)):
            axs[2].plot(t, zs[i], label=f'Line {i+1}')
    ...
    converge_visualization(np.array(trajectories_z1_followed_by_z2), "trajectories_z1_followed_by_z2")
    converge_visualization(np.array(trajectories_z1_stacking_with_z2), "trajectories_z1_stacking_with_z2")
